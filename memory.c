...
struct mempool* memory_allocate_mempool(struct device* dev, uint32_t num_entries, uint32_t entry_size) {
	...
	//debug("allocate mempool，block number is%d,block size is:%d",num_entries,entry_size);
...
static struct mempool* init_mempool(struct device* dev, uint32_t num_entries, uint32_t entry_size) { 
	...
	for (int buf_id = 0; buf_id < NUM_BUFS; buf_id++) {
		...
		while(true){
			...
			//debug("t = %ld",t);
			if(buf->size + pkt_size > S || t > T){
			    s += 64;
			    //debug("---------------------------------break in line 234");
			    break;
			}
			strncat(buf->data, pkt_data, pkt_size);
			//debug("i have done");
			buf->size += 64;
			s += 64;
		}
...
static void refull_mempool(struct mempool* mempool, uint32_t num_entries) {    //refull mempool
	...
	for (int buf_id = 0; buf_id < NUM_BUFS; buf_id++) {
		...
		//debug("t = %ld,T_l = %ld, T_u = %ld",t,T_l,T_u);
		if((t <= T_l || t >= T_u) && t > 20){
			//debug("%ld",t);
			T = t;
			T_l = T * 0.5;
		...
		while(true){
			...
			t = this_time - last_time;
			//debug("t = %ld",t);
			//debug("buf->size = %d",buf->size);
			if(buf->size + pkt_size > S || t > T){
			    s += 64;
			    //debug("-----------------------------break in line 541");
			    //debug("buf->size = %d, s = %d, S = %d", buf->size, s, S);
			    //debug("t = %ld, T = %ld", t, T);
			    break;
			}
			strncat(buf->data, pkt_data, pkt_size);
...
static void refull_mempool1(struct mempool* mempool, uint32_t num_entries) {
	...
	for (int buf_id = 0; buf_id < NUM_BUFS; buf_id++) {
		...
		while(true){
			...
			t = this_time - last_time;
			//debug("t = %ld",t);
			if(buf->size + pkt_size1 > S || t > T){
			    s += 256;
			    //debug("buf->size = %d, s = %d, S = %d", buf->size, s, S);
			    //debug("-------------------------------------break in line 589");
			    break;
			}
			strncat(buf->data, pkt_data, pkt_size);
			//debug("i have done");
